...
class SupportAIViewTests(ApiViewBaseTestCase):
    ...
    @patch("api.views_folder.support.get_escalation_count_today", return_value=0)
    @patch("api.views_folder.support.get_ai_response", return_value=("general", "Helpful answer", 0.91, False))
    ...
    @patch("api.views_folder.support.get_escalation_count_today", return_value=0)
    @patch("api.views_folder.support.get_ai_response", side_effect=RuntimeError("backend offline"))
    ...
    @patch("api.views_folder.support.logger.warning")
    @patch("api.views_folder.support.send_ticket_email.delay")
    @patch("api.views_folder.support.async_to_sync")
    @patch("api.views_folder.support.get_channel_layer")
    @patch("api.views_folder.support.create_structured_ticket")
    @patch("api.views_folder.support.extract_ticket_structure_smart", return_value={"priority": "normal"})
    @patch("api.views_folder.support.get_ai_response", return_value=("billing", "Escalating now", 0.3, True))
    @patch("api.views_folder.support.get_escalation_count_today", return_value=2)
    ...
    @patch("api.views_folder.support.get_channel_layer")
    @patch("api.views_folder.support.create_structured_ticket")
    @patch("api.views_folder.support.extract_ticket_structure_smart", return_value={"priority": "high"})
    @patch("api.views_folder.support.get_ai_response", return_value=("billing", "Escalating now", 0.2, True))
    @patch("api.views_folder.support.get_escalation_count_today", return_value=1)
